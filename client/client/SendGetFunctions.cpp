...
bool Client::GetPacketType(PacketType& pack_type)
{
	int packettype;
	if (!Getint32_t(packettype))
		return false;
	pack_type = (PacketType)packettype;
	return true;
}
...
bool Client::SendPacketType(PacketType pack_type)
{
	if (!Sendint32_t((int32_t)pack_type))
		return false;
	return true;
}
